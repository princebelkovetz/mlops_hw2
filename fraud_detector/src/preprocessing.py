...
def load_train_data():
    ...
    logger.debug('Raw train data imported. Shape: %s', train.shape)
...
def run_preproc(train, input_df):
    ...
    logger.info('Added distance features. Output shape: %s', input_df.shape)

    num_cols = ['amount', 'population_city', 'distance']

    # Impute empty values with mean value
    imputer = SimpleImputer(missing_values=np.nan, strategy='mean') 
    imputer = imputer.fit(train[num_cols])

    output_df = pd.concat([
        input_df.drop(columns=num_cols),
        pd.DataFrame(imputer.transform(input_df.copy()[num_cols]), columns=num_cols)
    ], axis=1)
    
    # Return resulting dataset
    logger.info(f"Input file preprocessed successfully.\nUsing features for prediction:\nCategorical: {cat_cols}\nNumerical: {num_cols}")
    return output_df[cat_cols + num_cols]